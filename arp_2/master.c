...
//いくつかの子プロセスを生成し、それぞれが異なるプログラムを実行することでプロセス間通信を行うサンプル
//各プロセスを実行するだけだよ
...
//fork を使用して子プロセスを生成し、それぞれが異なるプログラムを実行します。
//各子プロセスの実行には execvp を使用しています。
    child_server = fork(); //-1 エラー　0 fork as child　+ fork and return to parent
    if (child_server < 0) {
        perror("Fork");
        return -1;
    }
